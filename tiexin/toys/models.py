...
def get_fullsize_image_path(instance, filename):
	return '/'.join(['toys', 'source', instance.uuid, 'fullsize', filename])

def get_thumbnail_image_path(instance, filename):
	return '/'.join(['toys', 'source', instance.uuid, 'thumbnail', filename])
...
class ToySource(models.Model):
	...
	is_final_quote_accepted = models.NullBooleanField()
